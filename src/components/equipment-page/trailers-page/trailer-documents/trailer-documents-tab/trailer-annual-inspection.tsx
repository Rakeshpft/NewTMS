...
const AnnualInspectionDocuments = (props: TtrailerProps) => {
    ...
    const handleSaveDocument = (event: React.ChangeEvent<HTMLFormElement>) => {
        ...
        else if (trailerDocument.file || trailerDocument.document_id > 0) {
            postAnnualInspectionDocument(trailer_id, trailerDocument).then((data: any) => {
                data && toastify({ message: data.message, type: data.success ? "success" : "error", });
                data.success && getAnnualInspectionDocument(trailer_id);
                UploadModalClose();
            });
        }
    }
